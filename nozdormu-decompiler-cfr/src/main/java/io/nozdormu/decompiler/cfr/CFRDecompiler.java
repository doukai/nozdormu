...
public class CFRDecompiler implements TypeElementDecompiler {
  ...
  private boolean decompileAndCache(String decompileClassName) {
    ...
    OutputSinkFactory.Sink<SinkReturns.DecompiledMultiVer> decompiledSourceSink =
        decompiledList::add;
            ...
            switch (sinkType) {
              case JAVA:
                if (sinkClass != SinkClass.DECOMPILED_MULTIVER) {
                  throw new IllegalArgumentException(
                      "Sink class " + sinkClass + " is not supported for decompiled output");
                }
              ...
              case EXCEPTION:
                switch (sinkClass) {
                  ...
                  default:
                    throw new IllegalArgumentException(
                        "Sink factory does not support " + sinkClass);
                }
              case SUMMARY:
                return castSink(summarySink);
              default:
              ...
              .map(
                  decompiledMultiVer ->
                      new AbstractMap.SimpleEntry<>(
                          decompiledMultiVer.getPackageName()
                              + "."
                              + decompiledMultiVer.getClassName(),
