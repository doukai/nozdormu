    ...
    for (int i = 0; i < statementNodeList.size(); i++) {
      ...
      if (statement.isExpressionStmt()
        ...
        if (methodCallExpr.getScope().isPresent()
                && processorManager
                    .calculateType(methodCallExpr.getScope().get())
                    .asReferenceType()
                    .getQualifiedName()
        ...
        String methodDeclarationReturnTypeName =
            processorManager.resolveMethodDeclarationReturnTypeQualifiedName(methodCallExpr);
        String methodDeclarationReturnDescribe =
            processorManager.resolveMethodDeclarationReturnTypeDescribe(methodCallExpr);
        if (methodCallExpr.getScope().isPresent()
                && processorManager
                    .calculateType(methodCallExpr.getScope().get())
                    .asReferenceType()
            ...
            expression -> {
              if (expression.isMethodCallExpr()) {
                if (expression.asMethodCallExpr().getNameAsString().equals("await")
                  ...
                  String methodDeclarationReturnTypeName =
                      processorManager.resolveMethodDeclarationReturnTypeQualifiedName(
                          methodCallExpr);
                  if (methodDeclarationReturnTypeName.equals(Mono.class.getCanonicalName())) {
                    ...
                    String asyncMethodName =
                        Stream.concat(
                                ...
                                processorManager.resolveMethodDeclarationParameterTypeNames(
                                    methodCallExpr))
                            .collect(Collectors.joining("_"));
