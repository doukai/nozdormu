...
public class ProcessorManager {
  ...
  private Path getClassOutputPath() {
    try {
      FileObject tmp =
          filer.createResource(StandardLocation.CLASS_OUTPUT, "", UUID.randomUUID().toString());
      Writer writer = tmp.openWriter();
      writer.write("");
      writer.close();
      Path path = Paths.get(tmp.toUri());
      ...
      processingEnv
          .getMessager()
          .printMessage(Diagnostic.Kind.ERROR, "unable to determine class output path.");
      throw new RuntimeException(e);
    }
  }
  ...
  public String getQualifiedName(AnnotationExpr annotationExpr) {
    ResolvedAnnotationDeclaration resolvedAnnotationDeclaration =
        javaSymbolSolver.resolveDeclaration(annotationExpr, ResolvedAnnotationDeclaration.class);
    ...
    if (node.getParentNode().isPresent()) {
      ...
      if (parentNode instanceof Statement) {
        ...
        return compilationUnit.getImports().stream()
            ...
            .map(
                importDeclaration ->
                    getClassOrInterfaceDeclarationOrError(
                        importDeclaration
                            .getNameAsString()
                            .substring(0, importDeclaration.getNameAsString().lastIndexOf("."))))
            .findFirst()
            .flatMap(
                classOrInterfaceDeclaration ->
                    classOrInterfaceDeclaration
    ...
    if (methodCallExpr.hasScope()
      ...
      try {
        ResolvedReferenceType referenceType =
            calculateType(methodCallExpr.getScope().get()).asReferenceType();
        classOrInterfaceDeclaration =
            referenceType
                .getTypeDeclaration()
                .flatMap(
                    resolvedReferenceTypeDeclaration ->
                        resolvedReferenceTypeDeclaration.toAst(ClassOrInterfaceDeclaration.class))
                .orElseGet(
                    () ->
                        getPublicClassOrInterfaceDeclarationOrError(
                            getCompilationUnitOrError(referenceType.getQualifiedName())));
      } catch (RuntimeException e) {
        classOrInterfaceDeclaration =
            methodCallExpr
                .getScope()
      ...
      return classOrInterfaceDeclaration.getMethods().stream()
          .filter(
              methodDeclaration ->
                  methodDeclaration.getNameAsString().equals(methodCallExpr.getNameAsString()))
          .filter(
              methodDeclaration ->
                          ...
                          index -> {
                            try {
                              ResolvedType resolvedType =
                                  calculateType(methodCallExpr.getArgument(index));
                              if (resolvedType.isPrimitive()
                                  && methodDeclaration
                                      .getParameter(index)
                                      .getType()
                                ...
                                return resolvedType
                                    ...
                                    .toLowerCase()
                                    .equals(
                                        methodDeclaration.getParameter(index).getType().asString());
                              } else if (resolvedType.isReferenceType()
                                  && methodDeclaration
                                      .getParameter(index)
                                      .getType()
                                ...
                                return getResolvedType(
                                        methodDeclaration.getParameter(index).getType())
