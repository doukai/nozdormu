...
public class InjectProcessor extends AbstractProcessor {
  ...
  public synchronized void init(ProcessingEnvironment processingEnv) {
    ...
    ServiceLoader<ComponentProxyProcessor> loader =
        ServiceLoader.load(ComponentProxyProcessor.class, InjectProcessor.class.getClassLoader());
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    Set<? extends Element> applicationScopedSet =
        roundEnv.getElementsAnnotatedWith(ApplicationScoped.class);
    Set<? extends Element> requestScopedSet =
        roundEnv.getElementsAnnotatedWith(RequestScoped.class);
    Set<? extends Element> sessionScopedSet =
        roundEnv.getElementsAnnotatedWith(SessionScoped.class);
    Set<? extends Element> transactionScopedSet =
        roundEnv.getElementsAnnotatedWith(TransactionScoped.class);
    ...
    componentProxyProcessors.forEach(
        componentProxyProcessor -> {
            ...
            processorManager
                ...
                .ifPresent(
                    componentCompilationUnit -> {
                      ...
                      buildComponentProxy(componentCompilationUnit)
                          .ifPresent(
                              compilationUnit -> {
                                proxyCompilationUnitMap.put(
                                    typeElement.getQualifiedName().toString(), compilationUnit);
                                processorManager.writeToFiler(compilationUnit, typeElement);
                              });
                      CompilationUnit proxySuppliersCompilationUnit =
                          buildProxySuppliers(componentCompilationUnit, proxyCompilationUnitMap);
                      processorManager.writeToFiler(proxySuppliersCompilationUnit, typeElement);
                    }));
  ...
  private Optional<CompilationUnit> buildComponentProxy(TypeElement typeElement) {
    return processorManager.getCompilationUnit(typeElement).flatMap(this::buildComponentProxy);
  }
  ...
  private Optional<CompilationUnit> buildComponentProxy(
    ...
    CompilationUnit proxyCompilationUnit =
        new CompilationUnit().addType(proxyClassDeclaration).addImport(Generated.class);
    ...
    processorManager.importAllClassOrInterfaceType(
        proxyClassDeclaration, componentClassDeclaration);

    List<FieldDeclaration> privateFieldDeclarationList =
            ...
            .filter(
                fieldDeclaration ->
                    componentClassDeclaration.getConstructors().stream()
                        ...
                        .anyMatch(
                            name -> fieldDeclaration.getVariable(0).getNameAsString().equals(name)))
            .map(FieldDeclaration::clone)
            .collect(Collectors.toList());

    privateFieldDeclarationList.forEach(proxyClassDeclaration::addMember);

    List<MethodDeclaration> injectMethodDeclarations =
        componentClassDeclaration.getMethods().stream()
            ...
            .collect(Collectors.toList());
    boolean hasInjectMethod =
        !injectMethodDeclarations.isEmpty()
            && !componentClassDeclaration.getConstructors().isEmpty();

    componentClassDeclaration
        .getConstructors()
        .forEach(
            constructorDeclaration -> {
              ConstructorDeclaration proxyConstructorDeclaration =
              ...
              injectMethodDeclarations.forEach(
                  methodDeclaration -> {
                    methodDeclaration
                        .getParameters()
                        .forEach(proxyConstructorDeclaration::addParameter);
                    blockStmt.addStatement(
                        new MethodCallExpr()
        ...
        .filter(
            annotationExpr ->
                proxyClassDeclaration.getAnnotations().stream()
                    .noneMatch(
                        exist -> exist.getNameAsString().equals(annotationExpr.getNameAsString())))
        .filter(
            annotationExpr -> {
    ...
    List<MethodDeclaration> observesMethods =
        componentClassDeclaration.getMethods().stream()
            ...
            .collect(Collectors.toList());
    List<MethodDeclaration> observesAsyncMethods =
        componentClassDeclaration.getMethods().stream()
            .filter(
                methodDeclaration ->
                    methodDeclaration.getParameters().stream()
    ...
    if (hasObservesMethods) {
      buildObserverInnerClasses(
          componentClassDeclaration,
          proxyCompilationUnit,
          proxyClassDeclaration,
          observesMethods,
          Observes.class,
          ScopeEventObserver.class);
    }
    if (hasObservesAsyncMethods) {
      buildObserverInnerClasses(
          ...
          proxyCompilationUnit,
          proxyClassDeclaration,
          observesAsyncMethods,
          ObservesAsync.class,
          ScopeEventAsyncObserver.class);
    }
    ...
    for (ComponentProxyProcessor componentProxyProcessor : componentProxyProcessors) {
      ...
      if (componentProxyProcessor.processComponentProxy(
          componentCompilationUnit,
          componentClassDeclaration,
          proxyCompilationUnit,
          proxyClassDeclaration)) {
    ...
    boolean proxyModified =
        hasInjectMethod || hasObservesMethods || hasObservesAsyncMethods || componentProxyProcessed;
    if (!proxyModified) {
  ...
  private void buildObserverInnerClasses(
      ClassOrInterfaceDeclaration componentClassDeclaration,
      CompilationUnit proxyCompilationUnit,
      ClassOrInterfaceDeclaration proxyClassDeclaration,
      List<MethodDeclaration> observesMethods,
    ...
    observesMethods.forEach(
        methodDeclaration -> {
          ClassOrInterfaceDeclaration observerClassDeclaration =
              new ClassOrInterfaceDeclaration()
                  .addModifier(Modifier.Keyword.PUBLIC, Modifier.Keyword.FINAL)
                  .addImplementedType(observerInterfaceClass.getSimpleName())
                  .setName(
          ...
          Optional<Parameter> observesParameter =
              methodDeclaration.getParameters().stream()
                  .filter(parameter -> parameter.isAnnotationPresent(observesAnnotationClass))
                  .findFirst();
          ...
          if (isMonoType(methodDeclaration.getType())) {
            proxyCompilationUnit.addImport(Void.class);

            MethodDeclaration onEvent =
                new MethodDeclaration()
                    .setName("onEventAsync")
                    .setModifiers(Modifier.Keyword.PUBLIC)
                    .addParameter(
                        new Parameter()
                            .setName(methodDeclaration.getParameter(0).getName())
                    ...
                    .addStatement(
                        new ReturnStmt(
                            new MethodCallExpr()
                                .setName(methodDeclaration.getName())
                                .addArgument(
                                    methodDeclaration.getParameter(0).getNameAsExpression()))));
  ...
  private boolean isMonoType(Type type) {
    ...
    return classOrInterfaceType.findCompilationUnit().stream()
        .flatMap(compilationUnit -> compilationUnit.getImports().stream())
        .map(NodeWithName::getNameAsString)
        .anyMatch(
            importName ->
                importName.equals(Mono.class.getName())
                    || importName.equals(Mono.class.getPackageName() + ".*"));
  }

  private CompilationUnit buildProxySuppliers(
      CompilationUnit componentCompilationUnit,
      Map<String, CompilationUnit> proxyCompilationUnitMap) {
    ClassOrInterfaceDeclaration componentClassDeclaration =
        processorManager.getPublicClassOrInterfaceDeclarationOrError(componentCompilationUnit);
    String qualifiedName = processorManager.getQualifiedName(componentClassDeclaration);

    logger.info("{} proxy suppliers class build start", qualifiedName);

    ClassOrInterfaceDeclaration suppliersClassDeclaration =
        new ClassOrInterfaceDeclaration()
            .addModifier(Modifier.Keyword.PUBLIC)
            .setName(componentClassDeclaration.getNameAsString() + "_BeanSuppliers")
            .addAnnotation(
                new NormalAnnotationExpr()
                    .addPair("value", new StringLiteralExpr(getClass().getName()))
                    .setName(Generated.class.getSimpleName()))
    ...
    CompilationUnit suppliersCompilationUnit =
        new CompilationUnit()
            .setPackageDeclaration(
                componentCompilationUnit.getPackageDeclaration().orElseThrow().getNameAsString())
...

    {
      ...
      staticInitializer.addStatement(
          new VariableDeclarationExpr()
              .addVariable(
                  new VariableDeclarator()
                      .setName(componentPrefix + "_beanSupplier")
      ...
      Expression objectCreateExpression =
          Optional.ofNullable(proxyCompilationUnitMap.get(qualifiedName))
              .flatMap(
                  compilationUnit ->
              ...
              .map(
                  proxyClassDeclaration ->
                      proxyClassDeclaration.getMethods().stream()
                          ...
                          .filter(
                              methodDeclaration ->
                                  methodDeclaration.isAnnotationPresent(Produces.class))
                          .filter(
                              producesMethodDeclaration ->
                                  processorManager
                                      .getQualifiedName(producesMethodDeclaration.getType())
                                      .equals(qualifiedName + "_Proxy"))
                          .findFirst()
                          .map(
                              producesMethodDeclaration ->
                                  (Expression)
                                      new MethodCallExpr()
                                          ...
                                          .setArguments(
                                              producesMethodDeclaration.getParameters().stream()
                                                  ...
                                                  .map(
                                                      methodCallExpr -> (Expression) methodCallExpr)
                                                  .collect(Collectors.toCollection(NodeList::new)))
      ...
      if (componentClassDeclaration.isAnnotationPresent(Singleton.class)
          || componentClassDeclaration.isAnnotationPresent(ApplicationScoped.class)) {
        ClassOrInterfaceDeclaration holderClassDeclaration =
            new ClassOrInterfaceDeclaration()
                .setName(componentPrefix + "Holder")
                .setModifiers(Modifier.Keyword.PRIVATE, Modifier.Keyword.STATIC);

        holderClassDeclaration
            .addFieldWithInitializer(qualifiedName, "INSTANCE", objectCreateExpression)
            .setModifiers(
      ...
      } else if (componentClassDeclaration.isAnnotationPresent(RequestScoped.class)
          || componentClassDeclaration.isAnnotationPresent(SessionScoped.class)
          || componentClassDeclaration.isAnnotationPresent(TransactionScoped.class)) {
        String scopedAnnotationName =
            processorManager
                .getScopedAnnotationName(componentClassDeclaration)
                .orElseThrow(
                    () -> new InjectionProcessException(ANNOTATION_NOT_EXIST.bind(qualifiedName)));

        suppliersCompilationUnit.addImport(ReactorBeanScoped.class).addImport(Mono.class);
      ...
      componentClassDeclaration.getMethods().stream()
          ...
          .filter(
              producesMethodDeclaration ->
                  producesMethodDeclaration.isAnnotationPresent(Singleton.class)
                      || producesMethodDeclaration.isAnnotationPresent(ApplicationScoped.class))
          .forEach(
              producesMethodDeclaration -> {
                String methodTypeQualifiedName =
                    processorManager.getQualifiedName(producesMethodDeclaration);
                ...
                holderClassOrInterfaceDeclaration
                    .setName(
                        producesPrefix
                            ...
                            + componentClassDeclaration
                                .getMethods()
                                .indexOf(producesMethodDeclaration))
                    .setModifiers(Modifier.Keyword.PRIVATE, Modifier.Keyword.STATIC)
                                    ...
                                    .map(
                                        parameter ->
                                            getBeanGetMethodCallExpr(
                                                suppliersCompilationUnit,
                                                parameter,
                                                parameter.getType().asClassOrInterfaceType()))
          ...
          .forEach(
              annotationExpr -> {
                if (processorManager.hasMetaAnnotation(annotationExpr, Qualifier.class.getName())) {
                  ...
                  mapOf.addArgument(
                      qualifierToExpression(annotationExpr, suppliersCompilationUnit));
                }
              });
      ...
      componentClassDeclaration
          ...
          .ifPresent(
              annotation ->
                  applyPriorityAnnotation(
                      suppliersCompilationUnit,
                      staticInitializer,
                      componentPrefix + "_beanSupplier",
                      annotation));
                ...
                .setScope(
                    new MethodCallExpr()
                        ...
                        .addArgument(
                            new LambdaExpr()
                                .addParameter(
                                    new Parameter().setName("k").setType(new UnknownType()))
                                .setBody(
                                    new ExpressionStmt(
                                        new ObjectCreationExpr()
      ...
      processorManager
          .getExtendedTypes(componentClassDeclaration)
          .forEach(
              extendedTypeName ->
                  staticInitializer.addStatement(
                      new MethodCallExpr()
      ...
      processorManager
          .getImplementedTypes(componentClassDeclaration)
          .forEach(
              implementedTypeName ->
                  staticInitializer.addStatement(
                      new MethodCallExpr()
      ...
      componentClassDeclaration.getMethods().stream()
          ...
          .forEach(
              producesMethodDeclaration -> {
                ...
                MethodCallExpr mapOfProduces =
                    new MethodCallExpr().setName("of").setScope(new NameExpr("Map"));

                producesMethodDeclaration
                    .getAnnotations()
                    .forEach(
                        annotationExpr -> {
                          if (processorManager.hasMetaAnnotation(
                              annotationExpr, Qualifier.class.getName())) {
                            mapOfProduces.addArgument(
                                new StringLiteralExpr(
                                    processorManager.getQualifiedName(annotationExpr)));
                            mapOfProduces.addArgument(
                                qualifierToExpression(annotationExpr, suppliersCompilationUnit));
                          }
                        });
                              ...
                              new NameExpr(
                                  producesPrefix
                                      ...
                                      + componentClassDeclaration
                                          .getMethods()
                                          .indexOf(producesMethodDeclaration)));
                } else if (producesMethodDeclaration.isAnnotationPresent(RequestScoped.class)
                    || producesMethodDeclaration.isAnnotationPresent(SessionScoped.class)
                    || producesMethodDeclaration.isAnnotationPresent(TransactionScoped.class)) {
                  String scopedAnnotationName =
                      processorManager
                                                          ...
                                                          new ClassExpr()
                                                              .setType(
                                                                  processorManager.getQualifiedName(
                                                                      componentClassDeclaration)))
                                                      .setScope(
                                                          new NameExpr().setName("BeanContext"))))))
                          .setScope(
                              new MethodCallExpr()
                                  .setName("get")
                                  .addArgument(new ClassExpr().setType(ReactorBeanScoped.class))
                                      ...
                                      new MethodCallExpr()
                                          ...
                                          .addArgument(
                                              new MethodCallExpr()
                                                  ...
                                                  .addArgument(new StringLiteralExpr("value"))
                                                  .addArgument(
                                                      new StringLiteralExpr(scopedAnnotationName))
                                                  .setScope(new NameExpr("Map")))
                                          .setScope(new NameExpr("Map")))
                                  .setScope(new NameExpr(BeanContext.class.getSimpleName())));
                } else {
                ...
                staticInitializer.addStatement(
                    new VariableDeclarationExpr()
                        .addVariable(
                            new VariableDeclarator()
                                .setName(producesPrefix + "_beanSupplier")
                                .setType(BeanSupplier.class)
                                .setInitializer(
                                    new ObjectCreationExpr().setType(BeanSupplier.class))));
                ...
                processorManager
                    .getClassOrInterfaceDeclaration(methodTypeQualifiedName)
                    .ifPresent(
                        returnTypeClassOrInterfaceDeclaration -> {
                          processorManager
                              .getExtendedTypes(returnTypeClassOrInterfaceDeclaration)
                              .forEach(
                                  extendedTypeName ->
                                      staticInitializer.addStatement(
                                          new MethodCallExpr()
                                              .setName("put")
                                              .addArgument(
                                                  new StringLiteralExpr(methodTypeQualifiedName))
                                              .addArgument(
                                                  new NameExpr(producesPrefix + "_beanSupplier"))
                                              .setScope(
                                                  new MethodCallExpr()
                                                      .setName("computeIfAbsent")
                                                      .addArgument(
                                                          new StringLiteralExpr(extendedTypeName))
                                                      .addArgument(
                                                          new LambdaExpr()
                                                              .addParameter(
                                                                  new Parameter()
                          ...
                          processorManager
                              .getImplementedTypes(returnTypeClassOrInterfaceDeclaration)
                              .forEach(
                                  implementedTypeName ->
                                      staticInitializer.addStatement(
                                          new MethodCallExpr()
      ...
      buildObserverSuppliers(
          suppliersCompilationUnit,
          suppliersClassDeclaration,
          componentClassDeclaration,
          qualifiedName,
          staticInitializer,
          Observes.class,
    ...
    logger.info("{} proxy suppliers class build success", qualifiedName);
    return suppliersCompilationUnit;
  }

  private void buildObserverSuppliers(
      CompilationUnit suppliersCompilationUnit,
      ClassOrInterfaceDeclaration suppliersClassDeclaration,
      ClassOrInterfaceDeclaration componentClassDeclaration,
      String qualifiedName,
      BlockStmt staticInitializer,
      Class<? extends Annotation> observesAnnotationClass,
      Class<?> observerInterfaceClass) {
    List<MethodDeclaration> observesMethods =
        componentClassDeclaration.getMethods().stream()
            .filter(
                methodDeclaration ->
                    methodDeclaration.getParameters().stream()
                        .anyMatch(
                            parameter -> parameter.isAnnotationPresent(observesAnnotationClass)))
            .collect(Collectors.toList());
    ...
    suppliersCompilationUnit.addImport(observerInterfaceClass);
    ...
    observesMethods.forEach(
        methodDeclaration -> {
          ...
          holderClassOrInterfaceDeclaration
              ...
              .setModifiers(Modifier.Keyword.PRIVATE, Modifier.Keyword.STATIC)
              .addFieldWithInitializer(
                  observerInterfaceClass.getSimpleName(),
                  "INSTANCE",
                  new ObjectCreationExpr()
                      .setType(new ClassOrInterfaceType().setName(prefix))
                      .setScope(
                          new MethodCallExpr()
                              .setName("get")
          ...
          staticInitializer.addStatement(
              new MethodCallExpr()
                  ...
                  .setScope(
                      new MethodCallExpr()
                          .setName("computeIfAbsent")
                          .addArgument(new StringLiteralExpr(observerInterfaceClass.getName()))
                          .addArgument(
                              new LambdaExpr()
                                  .addParameter(
                                      new Parameter().setName("k").setType(new UnknownType()))
  ...
  private void applyPriorityAnnotation(
      CompilationUnit suppliersCompilationUnit,
      BlockStmt staticInitializer,
      String beanSupplierName,
      AnnotationExpr annotation) {
    if (annotation.isNormalAnnotationExpr()) {
      Expression valueExpression = annotation.asNormalAnnotationExpr().getPairs().get(0).getValue();
      if (valueExpression.isNameExpr()) {
        processorManager
            .getMemberValueQualifiedName(valueExpression)
            .forEach(
                numberValueQualifiedName ->
                    suppliersCompilationUnit.addImport(
                        numberValueQualifiedName
                            + "."
                            + valueExpression.asNameExpr().getNameAsString(),
                        true,
                        false));
      } else {
        processorManager
      ...
      staticInitializer.addStatement(
          new MethodCallExpr()
              .setName("setPriority")
              .addArgument(valueExpression.clone())
              .setScope(new NameExpr(beanSupplierName)));
    } else if (annotation.isSingleMemberAnnotationExpr()) {
      Expression valueExpression = annotation.asSingleMemberAnnotationExpr().getMemberValue();
      if (valueExpression.isNameExpr()) {
        processorManager
            .getMemberValueQualifiedName(valueExpression)
            .forEach(
  ...
  private Expression qualifierToExpression(
      AnnotationExpr qualifier, CompilationUnit compilationUnit) {
    MethodCallExpr mapOf = new MethodCallExpr().setName("of").setScope(new NameExpr("Map"));
    if (qualifier.isNormalAnnotationExpr()) {
              ...
              pair -> {
                if (compilationUnit != null) {
                  if (pair.getValue().isNameExpr()) {
                    processorManager
                        .getMemberValueQualifiedName(pair.getValue())
                        .forEach(
                            qualifiedName ->
                                compilationUnit.addImport(
                                    qualifiedName
                                        + "."
                                        + pair.getValue().asNameExpr().getNameAsString(),
                                    true,
                                    false));
                  } else {
                    processorManager
      ...
      if (compilationUnit != null) {
        if (qualifier.asSingleMemberAnnotationExpr().getMemberValue().isNameExpr()) {
          processorManager
              .getMemberValueQualifiedName(
                  qualifier.asSingleMemberAnnotationExpr().getMemberValue())
              .forEach(
                  qualifiedName ->
                      compilationUnit.addImport(
                          qualifiedName
  ...
  private MethodCallExpr getBeanGetMethodCallExpr(
      CompilationUnit proxyCompilationUnit,
    ...
    if (qualifiedName.equals(Provider.class.getName())) {
      Type type =
          classOrInterfaceType
              .getTypeArguments()
              .orElseThrow(() -> new InjectionProcessException(PROVIDER_TYPE_NOT_EXIST))
              .get(0);
      if (type.isClassOrInterfaceType()
          && processorManager.getQualifiedName(type).equals(Mono.class.getName())) {
        methodCallExpr =
            new MethodCallExpr()
                        ...
                        .setType(
                            processorManager.getQualifiedName(
                                type.asClassOrInterfaceType()
                                    ...
                                    .orElseThrow(
                                        () ->
                                            new InjectionProcessException(INSTANCE_TYPE_NOT_EXIST))
                                    .get(0))));
